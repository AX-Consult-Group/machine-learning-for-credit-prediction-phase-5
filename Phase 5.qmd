...
V2 Scoring (Challenger): V2 is the refined, regulator-ready hybrid scorecard developed in Phase 4. It incorporates the full set of 13 DRA higher-order factors with standardisation (centering and scaling) and six high-value interaction terms using a penalised Elastic Net regression.
...
p1 <- ggplot(v1_plot_data, aes(x = score_mid)) +
  ...
  scale_y_continuous(
    name = "Bad Rate (%)",
    sec.axis = sec_axis(~ . * max(v1_plot_data$n) / 45, name = "Volume (number of applicants)")
  ) +
  labs(
    title = "Actual vs Predicted Bad Rate by Score Band (V1)",
    subtitle = "Gini 10.74%",
    x = "Total Risk Score (Higher = Lower Risk)"
  ) +
...
p2 <- ggplot(v2_plot_data, aes(x = score_mid)) +
  ...
  scale_y_continuous(
    name = "Bad Rate (%)",
    sec.axis = sec_axis(~ . * max(v2_plot_data$n) / 45, name = "Volume (number of applicants)")
  ) +
  labs(
...
p3 <- ggplot(plot_data3, aes(x = Metric, y = Value, fill = group)) +
  ...
  scale_fill_manual(values = c("V1" = "#3B1F5E", 
                               "V2" = "#C4A8E8")) +
